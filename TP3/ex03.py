#python main.py /home/runner/ORI/TP3/hinos/vocabulary.txt /home/runner/ORI/TP3/hinos /home/runner/ORI/TP3/hinos/query.txt
...
def returnBoWFromFilePathWithoutStopWords(path, stopWords):
	file = open(path, "r")
...
def returnBoWFromFilePathWithoutStopWordsWithStemming(path, stopWords):
	file = open(path, "r")
	...
	for word in bagOfWords:
		if word not in stopWords:
			[tokenizedWord] = sent_tokenize(word, "portuguese")
			stemWord = stemmer.stem(tokenizedWord)
			stemWord = unidecode.unidecode(stemWord)
			newBagOfWords.append(stemWord)
...
for file in os.listdir():
	if file.endswith('.txt') and file != vocabularyFileName and file != queryFileName:
		...
		boW = returnBoWFromFilePathWithoutStopWordsWithStemming(file_path, stopwords)
...
vocabularyWithStemming = returnBoWFromFilePathWithoutStopWordsWithStemming(vocabularyPath, stopwords)
query = returnBoWFromFilePathWithoutStopWordsWithStemming(queryPath, stopwords)

listOfTerm = []

for term in vocabularyWithStemming:
